...
@interface BeautyPicViewController ()<UIScrollViewDelegate>
{
...
-(void)setRequest{
    ...
    NSString *url = [NSString stringWithFormat:@"http://api.tool.chexun.com/mobile/downEventAlbumPhotoList.do?albumId=%@",self.myUrl];
...
-(void)makeScrollView{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(kScreenWidth * i, imageY , kScreenWidth, kScreenWidth / 1.5)];
        [_imageView.image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        [_imageView sd_setImageWithURL:[NSURL URLWithString:array[i]]];
        
        [_imageViewArray addObject:_imageView];
